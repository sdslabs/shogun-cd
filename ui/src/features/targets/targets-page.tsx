import { ChevronRight, RefreshCw, Search, Server, Waypoints } from "lucide-react"
...
export function TargetsPage() {
  ...
  const targets = useMemo(() => [...(targetsQuery.data ?? [])].filter((target) => !deferredSearch || [target.name, target.type, target.host, target.user].some((value) => value.toLowerCase().includes(deferredSearch))).sort((a, b) => a.name.localeCompare(b.name)), [deferredSearch, targetsQuery.data])
...
function TargetSkeleton() { return <div>{Array.from({ length: 5 }).map((_, index) => <div className="flex h-16 items-center gap-5 border-b px-4 last:border-0" key={index}><Skeleton className="size-8" /><Skeleton className="h-3 w-32" /><Skeleton className="ml-auto h-5 w-16" /><Skeleton className="h-3 w-52" /><Skeleton className="h-3 w-20" /></div>)}</div> }
