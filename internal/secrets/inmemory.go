...
type InMemorySecretService interface {
...
func NewInMemoryService() InMemorySecretService {
	s := InMemoryService{
...
func (s *InMemoryService) FetchSecret(name string) (string, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *InMemoryService) AddSecret(name string, value string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.SecretMap[name] = value
}

func (s *InMemoryService) DeleteSecret(name string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	delete(s.SecretMap, name)
}

func (s *InMemoryService) ResolveSecrets(input string) string {
